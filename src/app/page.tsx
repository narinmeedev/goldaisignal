        ...
        <section className="bg-[#0d141c]">
          <div className="mx-auto grid max-w-[1200px] gap-8 px-4 py-16 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:px-8 lg:py-24">
            ...
            <div className="public-panel p-6 sm:p-8">
              <div className="flex flex-wrap items-start justify-between gap-4 border-b border-ga-border pb-6"><div><p className="font-mono text-xs uppercase tracking-[0.14em] text-ga-muted">Pro monthly</p><p className="mt-2 text-xl font-semibold text-ga-text">สมาชิก PRO รายเดือน</p></div><span className="rounded-md border border-emerald-500/25 bg-emerald-500/8 px-3 py-1.5 text-xs font-medium text-emerald-400">ทดลองฟรี {TRIAL_DURATION_DAYS} วัน</span></div>
              <div className="py-6"><div className="flex items-end gap-2"><span className="font-mono text-4xl font-semibold text-ga-gold">{formatBaht(PROMOTIONAL_MONTHLY_PRICE_THB)}</span><span className="pb-1 text-sm text-ga-muted">/ เดือน</span></div><p className="mt-2 text-xs text-ga-muted">ราคาปกติ <span className="line-through">{formatBaht(REGULAR_MONTHLY_PRICE_THB)}</span> · ราคาช่วงเปิดตัว</p></div>
              ...
              <Link href="/checkout" className="public-button-primary mt-7 min-h-13 w-full px-5">เริ่มทดลองใช้งาน <ArrowRight className="h-4 w-4" /></Link><p className="mt-3 text-center text-xs text-ga-muted">ไม่ผูกบัตรในช่วงทดลองใช้งาน</p>
            </div>
          </div>
        </section>
      </main>
