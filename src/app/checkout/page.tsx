...
export default function CheckoutPage() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      const referredBy = typeof window !== 'undefined' ? (localStorage.getItem('referred_by') || '') : '';
      const res = await fetch('/api/auth/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, password, otp, verificationToken, referralCode: referredBy }),
      });
